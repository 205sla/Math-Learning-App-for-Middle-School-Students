...
namespace Michsky.MUIP
{
    public class PieChart : MaskableGraphic
    {
        ...
        public void ChangeName(int itemIndex, string itemValue)
        {
            chartData[itemIndex].name = itemValue;

            this.enabled = false;
            this.enabled = true;
        }

        public void ChangeColor(int itemIndex, Color32 itemValue)
        {
            chartData[itemIndex].color = itemValue;
        ...
        public void ClearChart()
        {
            if (chartData.Count <= 1)
                return;  // 첫 번째 항목만 있을 경우, 아무 것도 하지 않음

            // 1. 첫 번째 항목을 제외한 나머지 항목 삭제
            for (int i = chartData.Count - 1; i > 0; i--)
            {
                // 2. 각 항목의 인디케이터 삭제
                if (indicatorParent != null && indicatorParent.childCount > i)
                {
                    Destroy(indicatorParent.GetChild(i).gameObject);  // 해당 인디케이터 삭제
                }

                chartData.RemoveAt(i);  // chartData에서 해당 항목 삭제
            }

            // 3. 그래픽 업데이트 (첫 번째 항목을 제외한 나머지 항목을 삭제한 후 그래픽을 다시 그리도록 함)
            SetVerticesDirty();
        }
